...
def split_dataset(args, dataset, n, env_i, seed=0):
    """
    ...
    keys_1 = keys[:n]       # testing
    keys_2 = keys[n:]       # training    
    
    if args.task == "few-shot_domain_generalization" and "SplitDataset" not in str(type(dataset)) and env_i not in args.test_envs:
        train_img_lst = [dataset.samples[k][0] for k in keys_2]
        train_y_lst = [dataset.samples[k][1] for k in keys_2]
        sampled_keys = []
        for l in range(max(train_y_lst) + 1):
            label_mask = np.array(train_y_lst) == l
            label_index = torch.arange(len(train_y_lst))[label_mask][:args.n_shot]
            sampled_keys += torch.tensor(keys_2)[label_index].tolist()
        keys_2 = sampled_keys
        
        print(f"Few-shot DG: {train_img_lst[0].split('/')[2]} - Select {args.n_shot} shots. (total: {len(keys_2)} samples)")
...
def accuracy_with_texts(network, loader, weights, device, val_loss_flag=False):
    correct = 0
    total = 0
    weights_offset = 0
    loss = []
...
def accuracy_MDSDI(network, loader, weights, device):
    correct = 0
    total = 0
    weights_offset = 0
